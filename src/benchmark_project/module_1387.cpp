...
#include "module_1377.hpp"

namespace module_1387 {

template<typename T, size_t BufferSize = 179584>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 1387);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 1387;
        }
    }
};

void process_module_1387() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 1387 * 3.14159;
    });
    
    std::cout << "Module 1387 result: " << result << "\n";
}
