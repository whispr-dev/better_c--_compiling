...
#include <thread>
#include <future>
#include "module_500.hpp"
#include "module_491.hpp"

namespace module_501 {

template<typename T, size_t BufferSize = 66176>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 501);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 501;
        }
    }
};

void process_module_501() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 501 * 3.14159;
    });
    
    std::cout << "Module 501 result: " << result << "\n";
}
