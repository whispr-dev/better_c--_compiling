...
#include <thread>
#include <future>
#include "module_293.hpp"
#include "module_284.hpp"

namespace module_294 {

template<typename T, size_t BufferSize = 39680>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 294);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 294;
        }
    }
};

void process_module_294() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 294 * 3.14159;
    });
    
    std::cout << "Module 294 result: " << result << "\n";
}
