...
#include <thread>
#include <future>
#include "module_164.hpp"
#include "module_155.hpp"
#include "common_base.hpp"

namespace module_165 {

template<typename T, size_t BufferSize = 23168>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 165);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 165;
        }
    }
};

void process_module_165() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 165 * 3.14159;
    });
    
    std::cout << "Module 165 result: " << result << "\n";
}
