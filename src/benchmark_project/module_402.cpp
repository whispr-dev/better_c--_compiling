...
#include <thread>
#include <future>
#include "module_401.hpp"
#include "module_392.hpp"

namespace module_402 {

template<typename T, size_t BufferSize = 53504>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 402);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 402;
        }
    }
};

void process_module_402() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 402 * 3.14159;
    });
    
    std::cout << "Module 402 result: " << result << "\n";
}
