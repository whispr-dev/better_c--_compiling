...
#include <thread>
#include <future>
#include "module_331.hpp"
#include "module_322.hpp"

namespace module_332 {

template<typename T, size_t BufferSize = 44544>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 332);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 332;
        }
    }
};

void process_module_332() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 332 * 3.14159;
    });
    
    std::cout << "Module 332 result: " << result << "\n";
}
