...
#include "module_990.hpp"
#include "common_base.hpp"

namespace module_1000 {

template<typename T, size_t BufferSize = 130048>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 1000);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 1000;
        }
    }
};

void process_module_1000() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 1000 * 3.14159;
    });
    
    std::cout << "Module 1000 result: " << result << "\n";
}
