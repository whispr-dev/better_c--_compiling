...
#include <thread>
#include <future>
#include "module_179.hpp"
#include "module_170.hpp"
#include "common_base.hpp"

namespace module_180 {

template<typename T, size_t BufferSize = 25088>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 180);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 180;
        }
    }
};

void process_module_180() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 180 * 3.14159;
    });
    
    std::cout << "Module 180 result: " << result << "\n";
}
