...
#include <thread>
#include <future>
#include "module_321.hpp"
#include "module_312.hpp"

namespace module_322 {

template<typename T, size_t BufferSize = 43264>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 322);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 322;
        }
    }
};

void process_module_322() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 322 * 3.14159;
    });
    
    std::cout << "Module 322 result: " << result << "\n";
}
