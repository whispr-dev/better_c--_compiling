...
#include <thread>
#include <future>
#include "module_375.hpp"
#include "module_366.hpp"

namespace module_376 {

template<typename T, size_t BufferSize = 50176>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 376);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 376;
        }
    }
};

void process_module_376() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 376 * 3.14159;
    });
    
    std::cout << "Module 376 result: " << result << "\n";
}
