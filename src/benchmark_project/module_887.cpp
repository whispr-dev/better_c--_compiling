...
#include <thread>
#include <future>
#include "module_886.hpp"
#include "module_877.hpp"

namespace module_887 {

template<typename T, size_t BufferSize = 115584>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 887);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 887;
        }
    }
};

void process_module_887() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 887 * 3.14159;
    });
    
    std::cout << "Module 887 result: " << result << "\n";
}
