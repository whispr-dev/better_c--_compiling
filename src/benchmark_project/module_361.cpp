...
#include <thread>
#include <future>
#include "module_360.hpp"
#include "module_351.hpp"

namespace module_361 {

template<typename T, size_t BufferSize = 48256>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 361);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 361;
        }
    }
};

void process_module_361() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 361 * 3.14159;
    });
    
    std::cout << "Module 361 result: " << result << "\n";
}
