...
#include <thread>
#include <future>
#include "module_391.hpp"
#include "module_382.hpp"

namespace module_392 {

template<typename T, size_t BufferSize = 52224>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 392);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 392;
        }
    }
};

void process_module_392() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 392 * 3.14159;
    });
    
    std::cout << "Module 392 result: " << result << "\n";
}
