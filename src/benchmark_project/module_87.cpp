...
#include <thread>
#include <future>
#include "module_86.hpp"
#include "module_77.hpp"

namespace module_87 {

template<typename T, size_t BufferSize = 13184>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 87);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 87;
        }
    }
};

void process_module_87() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 87 * 3.14159;
    });
    
    std::cout << "Module 87 result: " << result << "\n";
}
