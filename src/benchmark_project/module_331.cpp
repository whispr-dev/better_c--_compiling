...
#include <thread>
#include <future>
#include "module_330.hpp"
#include "module_321.hpp"

namespace module_331 {

template<typename T, size_t BufferSize = 44416>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 331);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 331;
        }
    }
};

void process_module_331() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 331 * 3.14159;
    });
    
    std::cout << "Module 331 result: " << result << "\n";
}
