...
#include <thread>
#include <future>
#include "module_250.hpp"
#include "module_241.hpp"

namespace module_251 {

template<typename T, size_t BufferSize = 34176>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 251);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 251;
        }
    }
};

void process_module_251() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 251 * 3.14159;
    });
    
    std::cout << "Module 251 result: " << result << "\n";
}
