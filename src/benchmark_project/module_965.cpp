...
#include <thread>
#include <future>
#include "module_964.hpp"
#include "module_955.hpp"
#include "common_base.hpp"

namespace module_965 {

template<typename T, size_t BufferSize = 125568>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 965);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 965;
        }
    }
};

void process_module_965() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 965 * 3.14159;
    });
    
    std::cout << "Module 965 result: " << result << "\n";
}
