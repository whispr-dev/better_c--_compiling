...
#include <thread>
#include <future>
#include "module_393.hpp"
#include "module_384.hpp"

namespace module_394 {

template<typename T, size_t BufferSize = 52480>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 394);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 394;
        }
    }
};

void process_module_394() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 394 * 3.14159;
    });
    
    std::cout << "Module 394 result: " << result << "\n";
}
