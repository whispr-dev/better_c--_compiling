...
#include <thread>
#include <future>
#include "module_385.hpp"
#include "module_376.hpp"

namespace module_386 {

template<typename T, size_t BufferSize = 51456>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 386);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 386;
        }
    }
};

void process_module_386() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 386 * 3.14159;
    });
    
    std::cout << "Module 386 result: " << result << "\n";
}
