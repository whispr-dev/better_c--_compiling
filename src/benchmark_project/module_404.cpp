...
#include <thread>
#include <future>
#include "module_403.hpp"
#include "module_394.hpp"

namespace module_404 {

template<typename T, size_t BufferSize = 53760>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 404);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 404;
        }
    }
};

void process_module_404() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 404 * 3.14159;
    });
    
    std::cout << "Module 404 result: " << result << "\n";
}
