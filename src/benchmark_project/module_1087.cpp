...
#include <thread>
#include <future>
#include "module_1086.hpp"
#include "module_1077.hpp"

namespace module_1087 {

template<typename T, size_t BufferSize = 141184>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 1087);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 1087;
        }
    }
};

void process_module_1087() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 1087 * 3.14159;
    });
    
    std::cout << "Module 1087 result: " << result << "\n";
}
