...
#include <thread>
#include <future>
#include "module_312.hpp"
#include "module_303.hpp"

namespace module_313 {

template<typename T, size_t BufferSize = 42112>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 313);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 313;
        }
    }
};

void process_module_313() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 313 * 3.14159;
    });
    
    std::cout << "Module 313 result: " << result << "\n";
}
