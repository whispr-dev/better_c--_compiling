...
#include <thread>
#include <future>
#include "module_420.hpp"
#include "module_411.hpp"

namespace module_421 {

template<typename T, size_t BufferSize = 55936>
class DataProcessor {
private:
...
public:
    ...
    void initialize_buffer() {
        for (size_t j = 0; j < BufferSize; ++j) {
            buffer_[j] = static_cast<T>(j * 0.1 + 421);
        }
    }
    ...
    void heavy_computation() {
        for (size_t k = 0; k < BufferSize / 10; ++k) {
            buffer_[k] = std::sin(buffer_[k]) * std::cos(buffer_[k]) + 421;
        }
    }
};

void process_module_421() {
    DataProcessor<double> processor;
    processor.heavy_computation();
    
    auto result = processor.process_data([](double& val) {
        return val * val + 421 * 3.14159;
    });
    
    std::cout << "Module 421 result: " << result << "\n";
}
